...
public abstract class PartitionedStorageTests : TestBase<Partitions>, IAsyncLifetime
{
    ...
    {
        var secondItemId = await Partition.Save(ItemName, Item);
        var firstItemId = await Partition.Save(AlternativeItemName, Item);

        var scanAscending = new ScanOptions() { MaxItems = 1 };
        var scannedAscending = await Partition.Scan(scanAscending);
        Assert.Equal(firstItemId, scannedAscending.Single().Id);
        ...
        Assert.Equal(secondItemId, scannedDescending.Single().Id);
    }
